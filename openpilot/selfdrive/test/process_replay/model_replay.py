...
def model_replay(lr, frs):
  ...
  models = [m.modelV2 for m in modeld_msgs if m.which() == "modelV2"]
  if CHESTNUT:
    assert models and all(m.big for m in models), "Chestnut replay must run the big model without fallback"
  chestnut = any(m.big for m in models)
